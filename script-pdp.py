import requests, base64, time, threading, AnalyzeCorpusSentiment, csv


def writeToFile(tweet, location, entities, sentiment):
    fileOutput = csv.writer(open("tweets-csv-pdp.csv", "a"))
...
def main():
    # OAUTH PARAMETERS
    consumerKey = b'BudH7tlzcwsdfdHqEU7GAxvj9'
    consumerSecret = b'M472W99oebHpfuW68pU00W0AVYQ9r6h8DmlsPxXcTOiMjSDFIQ'
    ...
    while(True):
        ...
        r = requests.get("https://api.twitter.com/1.1/search/tweets.json?q=pdp&tweet_mode=extended&result_type=recent", headers=headers)
        try:
